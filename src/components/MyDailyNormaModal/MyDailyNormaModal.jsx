...
const defaultValues = { gender: "woman", weight: "20", time: "0", water: calcDailyNorma({ gender: "woman", weight: "20", time: "0" }) };
...
const MyDailyNormaModal = ({ isOpen, closeModal }) => {
  ...
  return (
    <ModalContainer isOpen={isOpen} onClose={closeModal} className={styles.modal} overlayClassName={styles.overlay}>
      <div>
        ...
        <div className={styles.formulaWrapper}>
          <p>
            For girl: <span className={styles.formula}>V=(M*0,03) + (T*0,4)</span>
          </p>
          <p>
            For man: <span className={styles.formula}>V=(M*0,04) + (T*0,6)</span>
          </p>
        </div>
        <p className={styles.formulaComment}>
          <span className={styles.formulaCommentStar}>*</span> V is the volume of the water norm in liters per day, M is your body weight, T is the time of active sports, or another type of activity commensurate in terms of loads (in the absence of these, you must set 0)
        </p>
        <Formik initialValues={formValues} validationSchema={addWaterSchema} onSubmit={handleSubmit} enableReinitialize>
          {({ values, errors, setFieldValue, setFieldError }) => (
            <Form>
              ...
              <Button
                ...
                onClick={() => {
                  setFieldValue("water", userIsTyping ? values.water : calcDailyNorma(values));
                  setFormValues((prev) => ({ ...prev, water: userIsTyping ? values.water : calcDailyNorma(values) }));
                }}
              >
                Save
              </Button>
